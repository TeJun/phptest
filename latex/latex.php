...
function RenderLatex($text,$size=125){
	$head = "";
	$latex = "";
	$tail = "";
	...
	while(strpos($text, "$$") > -1){
		...
		$tail = substr($text, strpos($text, "$$")+2);
		if(strpos($tail, "$$") > -1){
			$tmplatex = checkLatex(substr($tail, 0, strpos($tail, "$$")));
			$latex = nl2br(latex_content($tmplatex,$size));
			$tail = substr($tail, strpos($tail, "$$")+2);
		}
		$text = $head.$latex.$tail;
	}
	return $text;
}
...
function checkLatex($text){
 ...
 function latex_content($text, $density=120) {
    // --------------------------------------------------------------------------------------------------
    // adjust this to match your system configuration
    $latexrender_path = dirname(__FILE__);
	//"/home/soar/public_html/latexrender";
    $latexrender_path_http = "/Include/latex";
	...
	// Add by soar, set _formula_density
	$latex->setFormulaDensity($density);
